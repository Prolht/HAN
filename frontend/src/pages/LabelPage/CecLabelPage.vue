      ...
      <div class="sentence-list">
        <div class="sentence"
          ...
          <div class="">
            ...
            <label-submit-btn :is-dirty="isDirtyList[sentenceInd]"
              :user-id="userId"
              :sentence="sentence"
          ...
          <div class="others-work"
            v-show="isShowOthersList[sentenceInd]">
            <div v-for="(otherSentence, ind) in sentence.results"
              :key="ind">
              <span class="others-username">{{otherSentence.username}}</span>
              <span class="word"
                v-for="(word, ind) in otherSentence.result"
                :key="ind">{{ word | displaySpace }}</span>
            </div>
          </div>
          <div>
            <span class="word"
              ...
              @mousedown="!isMobile && startSelect($event, sentenceInd, wordInd)"
              @mouseup.stop="!isMobile && endSelect($event, sentenceInd, wordInd)"
              @mousemove="!isMobile && selectingWord(sentenceInd, wordInd)"
              @touchstart="isMobile && mobileTouchWord($event, sentenceInd, wordInd, word)"
...
import { getCecEntityTypes } from '@/service/apiV2'
...
export default {
  ...
  methods: {
    ...
    startSelect (event, sentenceInd, wordInd) {
      if (event.button === undefined || event.button !== 0 || this.isSelecting) {
        return false
      }
    ...
    endSelect (event, sentenceInd, wordInd) {
    ...
    isWordSelected (sentenceInd, wordInd) {
      // TODO 调用太多
      console.log('isWordSelected')
      var { wordFrom, wordEnd } = this
      var from = Math.min(wordFrom.wordInd, wordEnd.wordInd)
      var end = Math.max(wordFrom.wordInd, wordEnd.wordInd)
      return sentenceInd === wordFrom.sentenceInd && from <= wordInd && wordInd <= end
    },
    mobileTouchWord (event, sentenceInd, wordInd, word) {
      if (!this.isSelecting) {
      ...
      if (!this.isSelecting) {
        this.startSelect(event, sentenceInd, wordInd)
      } else {
        this.endSelect(event, sentenceInd, wordInd)
      }
    },
    mobileTouchend () {
  ...
  created () {
    getCecEntityTypes().then((res) => {
      let _this = this
      let entityTypes = res.data.data
      entityTypes.forEach((item) => {
        _this.contextMenuData.menulists.push(
          {
            key: item.key,
            fnHandler: 'labelEntity',
            btnName: item.name
          }
        )
      })
